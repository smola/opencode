...
Custom tools should continue returning a plain string from `execute`. If you want to shape the tool state shown in the UI, call `context.metadata(...)` inside `execute`:

```ts title=".opencode/plugins/custom-tools.ts"
async execute(args, context) {
  context.metadata({
    title: `Inspect ${args.foo}`,
    metadata: {
      foo: args.foo,
    },
  })
...
The final completed tool state preserves both the last `title` and the last `metadata` values set through `context.metadata(...)`.

If a custom tool does not have a specialized renderer, the default tool card now prefers the completed `title` when present and shows the completed string `output` in the fallback UI.

To opt into the Bash-like terminal card, set `metadata.presentation` to `{ type: "terminal" }`. Set `command` when you want to control the `$ ...` line shown above the terminal output.
...
async execute(args, context) {
  ...
  return "Plan: 3 to add, 0 to change, 0 to destroy"
}
```

When `command` is omitted, the terminal renderer falls back to the completed `title`, then the default generic label.
