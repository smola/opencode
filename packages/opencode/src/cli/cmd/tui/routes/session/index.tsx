...
import type { ExecTool } from "@/tool/exec"
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={props.part.tool === "exit"}>
          <Bash {...toolprops} />
        </Match>
...
function Bash(props: ToolProps<typeof BashTool>) {
  ...
  const command = createMemo(() => {
    ...
    const cmd = (props.metadata as any).command
  ...
  const title = createMemo(() => {
    const desc = props.input.description ?? (props.metadata.description as string) ?? "Shell"
...
function Exec(props: ToolProps<typeof ExecTool>) {
  const { theme } = useTheme()
  const isRunning = createMemo(() => props.part.state.status === "running")
  const output = createMemo(() => stripAnsi(props.metadata.output?.trim() ?? ""))
  const [expanded, setExpanded] = createSignal(false)
  ...
  return (
    <Switch>
      <Match when={props.metadata.output !== undefined}>
        <BlockTool
          title={`# ${props.input.description ?? "Exec"}`}
          part={props.part}
          spinner={isRunning()}
          onClick={overflow() ? () => setExpanded((prev) => !prev) : undefined}
        >
          <box gap={1}>
            <text fg={theme.text}>$ {props.input.command?.join(" ")}</text>
            <Show when={output()}>
              <text fg={theme.text}>{limited()}</text>
            </Show>
            <Show when={overflow()}>
